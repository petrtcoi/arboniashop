...
const RadiatorAllOptions: React.FC<RadiatorAllOptionsProps> = ({
  modelInit,
  colorInit,
  connectionInit,
  modelFreeze = true,
  ...
  const SECTIONS = range(
    global.sectionsDefaultMin,
    modelInit.sectionsMax || global.sectionsDefaultMax
  );
  ...
  const [connectionCurr, setConnectionCurr] =
    useState<ConnectionOrigin>(connectionInit);
  ...
  useEffect(() => {
    const sectionQntyString: string =
      sectionsCurr < 10
        ? "0" + sectionsCurr.toString()
        : sectionsCurr.toString();
    setRadiatorTitle(
      `Arbonia Column ${modelCurr.nameShort}-${sectionQntyString}, ${colorCurr.name}, ${connectionCurr.name}`
    );
      ...
      {modelFreeze && modelCurr && (
        <Box marginY="20px">
          <Typography sx={styles.standardText}>
            Выберете интересующие вас характеристики для модели радиатора{" "}
            {modelCurr.name}.
          </Typography>
          <Typography sx={styles.standardText}>
            Если вам нужна помощь в подборе радиаторов,то мы с радостью
            проконсультируем и поможем с выбором наиболее подходящих моделей
          </Typography>
        </Box>
      )}
      <Box
      ...
      >
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={2} textAlign={{ xs: "left", md: "center" }}>
            <Box marginTop="10px" />
            <Typography sx={styles.smallTextBold}>модель</Typography>
            <Box marginTop="10px" />
            {modelFreeze ? (
              ...
              <NativeSelect value={modelCurr.id} onChange={handleChangeModel}>
          ...
          <Grid item xs={12} sm={2} textAlign={{ xs: "left", md: "center" }}>
            <Box marginTop="10px" />
            <Typography sx={styles.smallTextBold}>число секций</Typography>
            <Box marginTop="10px" />
            <NativeSelect
              value={sectionsCurr}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                setSectionsCurr(+e.target.value)
              }
            >
              {SECTIONS.map((sec) => (
          ...
          <Grid item xs={12} md={4} textAlign={{ xs: "left", md: "center" }}>
            <Box marginTop="10px" />
            <Typography sx={styles.smallTextBold}>цвет</Typography>
            <Box marginTop="10px" />
            <SingleColor
              color={colorCurr}
              highlighted={false}
          ...
          <Grid item xs={12} sm={4} textAlign={{ xs: "left", md: "center" }}>
            <Box marginTop="10px" />
            <Typography sx={styles.smallTextBold}>подключение</Typography>
            <Box marginTop="10px" />
            <SingleConnection
              connection={connectionCurr}
              highlighted={false}
          ...
          <Grid item xs={12}>
            <Box marginTop="50px">
              ...
              <Box marginTop="10px" paddingLeft="10px">
                ...
                <Typography sx={styles.standardTextThin}>
                  мощность (ΔT=50°C):{" "}
                  {Math.floor(+modelCurr.dt50 * sectionsCurr)} Вт
                </Typography>
                <Typography sx={styles.standardTextThin}>
                  мощность (ΔT=60°C):{" "}
                  {Math.floor(+modelCurr.dt60 * sectionsCurr)} Вт
                </Typography>
              </Box>
              <Box marginTop="40px">
