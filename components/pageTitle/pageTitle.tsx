...
const PageTitle: React.FC<PageTitleProps> = ({ header, subheader, breadcrumbs }) => {
    ...
    return (
        <Box paddingY={ "10px" } paddingX={ { xs: "10px", md: "20px" } } sx={ { backgroundColor: "#f6f6f6", borderBottom: "1px solid #e5e5e5" } }>
                            ...
                            breadcrumbs?.map((item, index) => {
                                return (
                                    (index + 1) < breadcrumbs.length ?
                                        <Box key={ index } display="inline" itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">

                                            <Link href={ item.link }>
                                                <a style={ { textDecoration: "none" } } itemProp="item" >
                                                    <Typography sx={ { ...styles.smallTextThinLink } } display="inline" itemProp="name">
                                                        { item.title }
                                                    </Typography>
