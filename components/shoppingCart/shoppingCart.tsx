...
const ShoppingCart: React.FC<ShoppingCartProps> = () => {
  ...
  const [itemsQnty, setItemsQnty] = useState<number>(
    shoppingCartContext.state.items.length
  );
  useEffect(() => {
    setItemsQnty(
      shoppingCartContext.state.items.reduce(
        (acc: number, x) => acc + x.qnty,
        0
      )
    );
  }, [shoppingCartContext.state.items]);

  const [shoppingCartItemsLoaded, setShoppingCartItemsLoaded] =
