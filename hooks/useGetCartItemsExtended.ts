...
const useGetCartItemsExtended = ({ models, colors, connections }: UseGetCartItemsExtendedProps): { items: CartItemExtended[] } => {
    const shoppingCartContext = useContext(ShoppingCartContext)
    const { rateEuro } = useContext(CurrencyContext)

    const [items, setItems] = useState<CartItemExtended[]>(shoppingCartContext.state.items.map(item =>
        getCartItem({ item, models, colors, connections, currencyRate: rateEuro })))
    useEffect(() => {
        setItems(shoppingCartContext.state.items.map(item =>
            getCartItem({ item, models, colors, connections, currencyRate: rateEuro })))
