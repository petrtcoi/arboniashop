...
const metaDescription =
	'Полный гид по трубчатым радиаторам Arbonia: плюсы и минусы, расчёт мощности, размеры, число труб, подключение и выбор модели для квартиры или частного дома.'
		...
		<Box>
			...
			>
				...
				>
					...
					простенок. Это не абстрактный класс отопительных приборов, а конкретный товар: на{' '}
					<LinkText href='/'>главной странице магазина</LinkText> представлены трубчатые радиаторы Arbonia с официальной
					гарантией и складским наличием в Москве и Санкт-Петербурге. Главное при выборе - определить высоту, число труб, подключение и нужную
				...
				>
					...
					<LinkText href='/articles/epd'>EPD</LinkText> и заводской гарантией. Ниже в одной статье собраны
					преимущества, ограничения и практический алгоритм выбора, без необходимости сверять несколько обзоров.
				</Typography>
				...
				<Typography
					...
					sx={{ ...styles.standardText }}
				>
					Надёжный подбор идёт от условий эксплуатации к внешнему виду: сначала фиксируют параметры помещения и
					системы отопления, затем рассчитывают мощность, определяют допустимые размеры и число труб, выбирают
					подключение и только в конце - цвет. Такой порядок защищает от красивой, но недостаточно мощной или
					неподходящей для монтажа покупки.
				</Typography>
				...
				<Typography
					...
					marginBottom='8px'
				>
					По мощности и числу секций
				</Typography>
				<Typography
					component='p'
					sx={{ ...styles.standardText }}
				>
					Мощность считают для конкретной комнаты, а не выбирают по условной норме на один квадратный метр. На
					результат влияют площадь и высота потолка, число наружных стен, размер и тип остекления, утепление,
					этаж, регион и температура теплоносителя. После определения теплопотерь подбирают число секций по
					таблице теплоотдачи выбранной модели при том же температурном режиме, в котором будет работать система.
					Каталожные значения для разных режимов нельзя сравнивать напрямую. Пошаговый принцип расчёта разобран в{' '}
					<LinkText href='/articles/calculate'>руководстве по мощности радиатора</LinkText>.
				</Typography>
				<Typography
					component='p'
					marginTop='14px'
					sx={{ ...styles.standardText }}
				>
					Если длина стены ограничена, требуемую мощность можно получить более глубокой 3- или 4-трубчатой моделью.
					Если глубина критична из-за узкого подоконника или прохода, выбирают 2-трубчатую модель и увеличивают
					число секций. Поэтому размеры и мощность проверяют одновременно: короткий глубокий радиатор и длинный
					тонкий могут решать одну тепловую задачу, но по-разному вписываться в помещение.
				</Typography>
				...
				<Typography
					...
					marginBottom='10px'
				>
					Квартира или частный дом: что проверить
				</Typography>
				<Typography
					component='p'
					sx={{ ...styles.standardText }}
				>
					В квартире выбор чаще ограничен уже существующей разводкой и параметрами центрального отопления. В
					частном доме систему можно настроить под проект, но расчёт должен учитывать фактический температурный
					график котла. В обоих случаях совместимость подтверждают по техническому паспорту конкретной модели, а
					не только по названию серии.
				</Typography>
				<Box sx={{ overflowX: 'auto', marginTop: '12px' }}>
					<table style={tableStyle}>
						<thead>
							<tr>
								<th style={headCellStyle}>Условия</th>
								<th style={headCellStyle}>Что проверить до заказа</th>
								<th style={headCellStyle}>Практический приоритет</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td style={cellStyle}>Квартира с центральным отоплением</td>
								<td style={cellStyle}>Рабочее и испытательное давление, качество теплоносителя, разрешённую схему и межосевое расстояние труб</td>
								<td style={cellStyle}>Совместимость с системой дома и монтаж без переделки стояка</td>
							</tr>
							<tr>
								<td style={cellStyle}>Квартира с автономным отоплением</td>
								<td style={cellStyle}>Температуру подачи, гидравлику контура, место установки и доступ к регулировке</td>
								<td style={cellStyle}>Точный расчёт мощности при реальном режиме котла</td>
							</tr>
							<tr>
								<td style={cellStyle}>Частный дом</td>
								<td style={cellStyle}>Теплопотери каждого помещения, низкотемпературный режим, длину контуров и будущую отделку</td>
								<td style={cellStyle}>Согласовать радиаторы, котёл и разводку до вывода труб</td>
							</tr>
						</tbody>
					</table>
				</Box>
				...
				<Typography
					...
					marginBottom='10px'
				>
					Порядок выбора за 7 шагов
				</Typography>
				<Box
					component='ol'
					sx={{ pl: 3, mt: 1 }}
				>
					<li><Typography sx={{ ...styles.standardText }}>Запишите площадь, высоту потолка, число наружных стен, параметры окон и особенности утепления.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>Уточните тип системы, температуру теплоносителя, рабочее давление и существующую схему подключения.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>Рассчитайте необходимую мощность комнаты с поправками на реальные теплопотери.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>Измерьте доступную высоту, ширину и глубину, оставив монтажные зазоры сверху, снизу и у стены.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>Выберите высоту и число труб, затем определите число секций по теплоотдаче конкретной модели.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>Согласуйте боковое или нижнее подключение, расположение подачи, арматуру, крепления и тип стены.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>После технической проверки выбирайте цвет, срок поставки и окончательную комплектацию.</Typography></li>
				</Box>
				...
				<Box
					...
					sx={{ pl: 3, mt: 1 }}
				>
					<li><Typography sx={{ ...styles.standardText }}>Выбирать только по дизайну или площади комнаты, не считая остекление, наружные стены и температуру системы.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>Сравнивать мощность моделей, указанную для разных температурных режимов.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>Считать одинаковыми радиаторы одной высоты, не учитывая число труб, глубину и количество секций.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>Измерять только корпус и забывать про зазоры, кронштейны, термоголовку, нижний узел и чистовой пол.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>Вывести трубы до утверждения артикула, стороны подачи и монтажного чертежа.</Typography></li>
					<li><Typography sx={{ ...styles.standardText }}>Покупать цветную заказную модель до окончательной проверки размеров и подключения.</Typography></li>
				</Box>
				<Typography
					component='p'
					marginTop='14px'
					sx={{ ...styles.standardText }}
				>
					Для проверки подбора достаточно прислать план или размеры комнаты, высоту потолка, фотографию окна и
					существующих труб, сведения о системе отопления и желаемый цвет через{' '}
					<LinkText href='/request'>форму расчёта</LinkText>. Это позволяет проверить мощность, габариты и
					комплектацию до заказа, когда ошибки ещё не требуют переделки отделки.
				</Typography>
