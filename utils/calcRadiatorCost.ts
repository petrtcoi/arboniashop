...
const calcRadiatorCost: CalcRadiatorCost = ({
  model,
  color,
  connection,
  sectionQnty = global.sectionsDefaultMin,
  currencyRate,
}) => {
  ...
  const baseWithColorCost = color.baseRate
    ? baseCost * +color.baseRate
    : baseCost;
  ...
  const euroCostW161h121 =
    euroCostNoConsole + consoles.w161h121 * w161h121Price;
