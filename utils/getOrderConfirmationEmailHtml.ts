...
const getOrderConfirmationEmailHtml = (props: Props): string => {
  ...
  if (props.name.length > 0)
    html += `<span style="white-space:pre">Имя:&#9; ${ props.name }</span><br>`
  if (props.city.length > 0)
    html += `<span style="white-space:pre">Город:&#9; ${ props.city }</span><br>`
  if (props.address.length > 0)
    html += `<span style="white-space:pre">Адрес:&#9; ${ props.address }</span><br>`
  if (props.phone.length > 0)
    html += `<span style="white-space:pre">Тел:&#9;&#9; ${ props.phone }</span><br>`
  if (props.email.length > 0)
    html += `<span style="white-space:pre">Email:&#9; ${ props.email }</span><br>`
  // html += '</ul>'
  if (props.comments.length > 0)
    html += `<p>Комментарий к заказу:<br/>${ props.comments }</p>`
  ...
  items.forEach((item, index) => {
    html += `<tr><td>${ index + 1 }</td><td>${ item.title
      }</td><td>${ item.price.noConsole.toLocaleString() } руб.</td><td>${ item.qnty
      }</td><td>${ (
        item.price.noConsole * item.qnty
      ).toLocaleString() } руб.</td></tr> `
  })

  if (consoleType === "w161h121") {
    consoles.forEach((console) => {
      const consolePrice = Math.floor(console.totalSum / console.qnty)
      html += `<tr> <td></td><td>Кронштейн ${ console.title }, ${ console.colorName
        }</td> <td>${ consolePrice.toLocaleString() } руб.</td><td>${ console.qnty
        }</td> <td>${ console.totalSum.toLocaleString() } руб.</td></tr> `
    })
  }
  ...
  if (items.length > 0)
    html += `<p> Итого: <strong>${ getCartTotal({
      items,
      consoleType,
